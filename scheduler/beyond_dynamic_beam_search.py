...
class BeyondDynamicBeamSearchSchedulerAdvanced:
    """
    Patched and enhanced Beam Search Advanced scheduler.

    Improvements applied:
      - Dynamic/adaptive beam width adjustment
      - Iterative deepening over beam multipliers and optional depth limit
      - Enhanced backtracking (randomized window trials + multi-start refill)
      - Local search moves (replace / swap / shift)
      - Adaptive restarts and stagnation detection
      - Simple CSV logging support for experiments
      - Overlap handling + Genre diversity scoring (ADDED)

    Drop-in replacement: use the same constructor args as before with a few new optional tuning params.
    """
    ...
    def __init__(
        ...
        # internal tracker for dynamic beam
    ...
    def _validate_constructor_params(self) -> None:
        ...
        if not isinstance(self.backtrack_window, int) or self.backtrack_window <= 0:
            # backtrack_window 0 is allowed (means disabled) but we will enforce >=0
    ...
    def _dynamic_beam_adjustment(self, prev_score: Optional[float], current_score: float, current_width: int) -> int:
        """Adjust beam width adaptively: if no improvement increase exploration, else slightly shrink."""
    ...
    def _apply_adaptive_restart_policy(self):
        # Simple adaptation: if stagnation, nudge parameters to diversify search
        if self._detect_stagnation():
            # increase jump_cap a bit and allow a wider beam for next restarts
    ...
    def _refill_window(self, prefix: List[Schedule], window: int, trials: int = 1) -> Tuple[List[Schedule], int]:
        """Refill a window starting from prefix using bounded beam (like _backtrack_improve core)."""
        temp_sched = prefix.copy()
        local_beam = max(1, min(self.beam_width, 6))
        bs = self._create_scheduler(local_beam)
        fake_tail = [ScheduleModel(program_id="_pad", channel_id=-1, start=0, end=0, fitness=0, unique_program_id="_pad") for _ in range(window)]
    ...
    def _enhanced_backtrack(self, scheduled: List[Schedule], total_score: int, max_trials: int = 6) -> Tuple[List[Schedule], int]:
        """Try several randomized window replacements and multi-start prefixes to escape local optima."""
        n = len(scheduled)
        ...
        trials = min(max_trials, max(1, n))
        for _ in range(trials):
            window = random.randint(1, min(self.backtrack_window, n))
            start_idx = random.randint(0, max(0, n - window))
            key = (start_idx, window)
            if key in tried:
                continue
            tried.add(key)
    ...
    def _local_search_replace(self, sched: List[Schedule], deadline: Optional[float] = None) -> Tuple[List[Schedule], int]:
        """Try replacing individual schedule entries with other channel programs starting at same time."""
        best_sched = sched
    ...
    def _local_search_swap(self, sched: List[Schedule], deadline: Optional[float] = None) -> Tuple[List[Schedule], int]:
        """Try swapping two non-overlapping schedule entries to see if score improves."""
        best_sched = sched
        best_score = self._score_full_schedule(sched)
        n = len(sched)
        for i in range(n):
    ...
    def _apply_local_search(self, best_solution: Solution, deadline: Optional[float] = None) -> Solution:
        ...
        for _ in range(self.local_search_passes):
            if deadline and time.time() > deadline:
                break
            candidate_sched, candidate_score = self._local_search_replace(best_sched, deadline)
            if candidate_score > best_score:
                best_sched = candidate_sched
                best_score = candidate_score
                continue
            candidate_sched, candidate_score = self._local_search_swap(best_sched, deadline)
            if candidate_score > best_score:
                best_sched = candidate_sched
                best_score = candidate_score
                continue
    ...
    def generate_solution_with_time(self, restarts: int = 3, time_limit: Optional[float] = None) -> Solution:
        ...
        for r in range(restarts):
            ...
            jitter = max(0.9, random.uniform(0.9, 1.2))
            dynamic_width = max(1, int(current_dynamic_width * jitter))
            print(f"[Restart {r + 1}/{restarts}] Beam width = {dynamic_width}, Seed = {seed}")
            ...
            current_dynamic_width = self._dynamic_beam_adjustment(self._prev_restart_best, score, current_dynamic_width)
        ...
        try:
            if best_solution and self.backtrack_window > 0:
                refined_sched, refined_score = self._enhanced_backtrack(best_solution.scheduled_programs, int(best_solution.total_score), max_trials=8)
        ...
        if self.enable_logging and self.log_path:
            try:
                with open(self.log_path, 'a', encoding='utf-8') as f:
                    f.write(f"seed={self._last_seed},restarts={self._restarts_run},best_score={best_score}\n")
            except Exception:
                pass
    ...
    # ---------- NEW: lightweight lookups for genre/overlaps ----------
    def _lookup_program(self, channel_id: int, start_time: int):
        """Find the underlying program object for a (channel_id, start_time)."""
        try:
    ...
    def _get_program_genre(self, sched_entry: Schedule) -> Optional[str]:
        """
        Best-effort genre extraction with zero external changes:
        - Prefer underlying program's 'genre' or 'category'
        - Fall back to any genre-like attribute on the schedule entry
        """
        ...
        if prog is not None:
            ...
            if g:
                return str(g)
        # possible fallback if schedule entries carry genre
    ...
    def _score_full_schedule(self, schedule: List[Schedule]) -> int:
        """
        Compute the total score of a full schedule.
        Includes program fitness + penalties/bonuses (switch, delay, early termination).
        Adds:
          - Overlap handling (heavy penalty).
          - Genre diversity encouragement + anti-repetition penalty.
        Tunables (optional on instance_data; safe defaults provided):
          - overlap_penalty: int (default 10_000)
          - misorder_penalty: int (default 1_000)
          - genre_window: int (default 3)
          - genre_diversity_bonus: int (default 5) bonus per unique-genre-in-window
          - same_genre_chain_penalty: int (default 8) penalty per extra same-genre in a streak
        """
        if not schedule:
            return 0
        ...
        for idx, sch in enumerate(schedule):
            ...
            # Enforce chronological order & no overlaps on the chosen channel sequence
            if idx > 0:
                prev = schedule[idx - 1]

                # Mis-ordered entries (start earlier than previous end) → penalize
                if getattr(sch, "start", 0) < getattr(prev, "end", 0):
                    # If same channel, consider it an overlap (heavier)
                    if getattr(prev, "channel_id", None) == getattr(sch, "channel_id", None):
                        total -= overlap_penalty
                    else:
                        total -= misorder_penalty

                # Switch penalty if channel changes
                if getattr(prev, "channel_id", None) != getattr(sch, "channel_id", None):
            ...
            if g is not None:
                ...
                if g == last_genre:
                    same_genre_streak += 1
                    # penalize each extra in the streak (1st repeat counts as +1)
                    total -= same_genre_chain_penalty * (1)  # simple linear; keep minimal to avoid dominance
                else:
                    same_genre_streak = 0
                    last_genre = g

                # rolling window diversity bonus
                recent_genres.append(g)
                if len(recent_genres) > genre_window:
                    recent_genres.pop(0)
                # more unique genres in the small window → small positive
                unique_in_window = len(set([gg for gg in recent_genres if gg is not None]))
                # scale by uniqueness vs window size (normalized)
                total += genre_diversity_bonus * max(0, unique_in_window - 1)

            # (Optional) other penalties/bonuses available via AlgorithmUtils if needed
            # e.g., early termination / delay already included upstream during fitness build
