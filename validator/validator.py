...
class Validator:
    ...
    @staticmethod
    def is_channel_valid(schedule_plan: List[Schedule], instance_data: InstanceData, channel_index: int, schedule_time: int):
        try:
    ...
    def validate_min_duration(schedule_plan: List[Schedule], instance_data: InstanceData, schedule_time: int):
        ...
        if last_schedule.start + instance_data.min_duration < schedule_time:
            raise ConstraintException("min_duration for broadcasting channel has not been reached.")

    @staticmethod
    def validate_max_consecutive_genre(schedule_plan: List[Schedule], instance_data: InstanceData, channel_index: int, schedule_time: int):
        if not schedule_plan:
            return
        ...
        for schedule in reversed(schedule_plan):
            scheduled_program = Utils.get_program_by_unique_id(instance_data, schedule.program_id)
    ...
    def validate_priority_time_block(instance_data: InstanceData, channel_index: int, schedule_time: int):
        ...
        for block in instance_data.priority_blocks:
            if block.start <= schedule_time <= block.end and channel_to_insert_id not in block.allowed_channels:
